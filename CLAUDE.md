...
- `notz_player_cards.json`(플레이어) / `notz_encounter_cards.json`(조우: 적·음모) = 어떻게 작동하는가(abilities·zone·keywords). 여기만 채운다.
- 폭로(Revelation) 판별은 text 검색이 아니라 **`on_draw` 유무**로 한다.
- 효과 용어는 `card_schema_dictionary.md`를 따른다. 새 용어는 거기 먼저 등재.
- **★ 한글 텍스트 지침:** 카드를 작성할 때마다 cards.json `text`에 **한글이 없으면(영문)** notz 파일에 **`text_ko`(직접 번역)를 반드시 작성**한다. 카드명·특성은 항상 한글이라 **본문만** 판정. 게임의 툴팁·확대뷰는 `cardTextOf(code)`=`text_ko ‖ cards.json text`로 출력하므로, text_ko가 없으면 영문이 그대로 노출된다. (개정판 조사자 카드는 한글, core 조우 카드가 영문 → 주로 조우 카드가 대상)
...
## 모든 HTML 기본 UX 차단 (새 화면 만들 때 항상 포함)

브라우저 기본 상호작용(우클릭 메뉴·텍스트 선택 드래그·이미지 드래그 잔상)은 게임 몰입을
깨므로 **모든 화면에서 기본으로 막는다.** scenario1·arkham_game·notzCampaignLog 모두 적용됨.

```css
/* <style> — 텍스트 선택·이미지 드래그 차단 */
body{ user-select:none; -webkit-user-select:none; -moz-user-select:none; }
img{ -webkit-user-drag:none; user-drag:none; }
```
```js
/* 스크립트 최상위 — 우클릭 메뉴·이미지 드래그 차단 */
window.addEventListener("contextmenu", e=> e.preventDefault());
window.addEventListener("dragstart", e=>{ if(e.target && e.target.tagName==="IMG") e.preventDefault(); });
```
- 우클릭에 기능(카드 확대·팝업 취소 등)을 물릴 땐 `preventDefault()` 뒤에 그 처리를 잇는다(scenario1 참고).
- 게임 입력 화면은 추가로 `keydown`에서 Tab·Enter `preventDefault`(포커스 순회·버튼 오발 방지) — scenario1 전용, 메뉴/기록지엔 불필요.

## 나레이션·컷신 "계속하려면 아무거나 누르세요" (공통 패턴)

한 글자씩 출력이 끝나면 **자동 전환하지 말고**, 우상단 "스킵하려면 esc를 길게 누르세요"를
"계속하려면 아무거나 누르세요"로 바꾸고 **아무 키·클릭에 다음으로** 넘긴다(읽을 시간 보장).
arkham_game 프롤로그·scenario1 컷신·notzCampaignLog 에필로그가 같은 방식을 쓴다(출력 중엔 ESC 길게=스킵).
