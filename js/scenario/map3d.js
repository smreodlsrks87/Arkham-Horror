...
let D = {
  canAct:()=>false, actionSurchargeFor:()=>0, markSurcharge:()=>{}, takeDamageHorror:(d,h,o,cb)=>cb&&cb(false),
  effShroud:()=>0, leadInvestigatorName:()=>"", attachmentIcon:()=>"", attachmentLabel:()=>"",
  sweepAttachmentLeaves:()=>{}, askInvestigate:()=>{}, ritaParleyAvailable:()=>false, parleyRita:()=>{},
};
...
function focusRoom(k){
  ...
  if(ROOMS[k].stage==="mansion" && !S.revealedRooms[k]){
    ...
    if(info.onEnterHorror){ addLog("강제 — "+ROOMS[k].name+"에 들어와 공포 "+info.onEnterHorror+"을(를) 받습니다."); D.takeDamageHorror(0, info.onEnterHorror, {source:"location"}, ()=>{}); }
    if(info.onEnterDamage){ addLog("강제 — "+ROOMS[k].name+"에 들어와 피해 "+info.onEnterDamage+"을(를) 받습니다."); D.takeDamageHorror(info.onEnterDamage, 0, {source:"location"}, ()=>{}); }
  }
...
const barricadeEls = {};
const BARRICADE_SVG = '<svg viewBox="0 0 64 48" width="58" height="44">'+
...
function renderBarricades(){   // 마커 DOM 생성/제거 + 한 장소 여러 부착물 = 가로 배열(버프바처럼)
  ...
  Object.keys(S.locationAttachments).forEach(k=>{
    ...
    d.innerHTML = '<div class="bm-row">'+ atts.map(a=>'<div class="bm-item">'+D.attachmentIcon(a)+'<div class="bm-label">'+D.attachmentLabel(a)+'</div></div>').join("") +'</div>';   // 가운데 정렬로 가로 늘어놓기
  });
...
function updateBarricades(){
  ...
  Object.keys(S.locationAttachments).forEach(D.sweepAttachmentLeaves);
