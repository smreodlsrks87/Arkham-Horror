...
function runPhase(name, event, nextFn){
  if(S.scenarioOver) return;             // 시나리오 종료(조사자 사망·후퇴·결말) → 라운드 루프 정지
  setPhase(name);
  S.phasePaused = false;
  const g = ++advanceGen;
  bus.emit(event);
...
export function resumeEnemy(){ if(S.scenarioOver) return; advanceGen++; S.phasePaused=false; setTimeout(upkeepPhase, PHASE_HOLD); }
function upkeepPhase(){ runPhase("upkeep", "phase:upkeep", mythosPhase); }      // 4.2~4.5 정비(비전투)
// 손패 정리 끝나면 호출 → 신화 단계로
export function resumeUpkeep(){ if(S.scenarioOver) return; advanceGen++; S.phasePaused=false; setTimeout(mythosPhase, PHASE_HOLD); }
